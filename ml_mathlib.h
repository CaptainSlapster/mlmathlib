...
#ifndef ML_MATHLIB_H_
#define ML_MATHLIB_H_
...
static inline float ml_vec_2_mag_sq(ml_Vec2 v)                                           {return (v.x * v.x) + (v.y * v.y); }
static inline float ml_vec_3_mag_sq(ml_Vec3 v)                                           {return (v.x * v.x) + (v.y * v.y) + (v.z * v.z); }


static inline ml_Vec2 ml_vec_2_scalar_add(ml_Vec2 v, float s)                            {return (ml_Vec2){v.x + s, v.y + s}; }
static inline ml_Vec2 ml_vec_2_scalar_sub(ml_Vec2 v, float s)                            {return (ml_Vec2){v.x - s, v.y - s}; }
static inline ml_Vec3 ml_vec_3_scalar_sub(ml_Vec3 v, float s)                            {return (ml_Vec3){v.x - s, v.y - s, v.z - s}; }
static inline ml_Vec3 ml_vec_3_scalar_add(ml_Vec3 v, float s)                            {return (ml_Vec3){v.x + s, v.y + s, v.z + s}; }
...
#define ml_vec_mag_sq(v) _Generic((v),             \
    ml_Vec2:          ml_vec_2_mag_sq,                \
    ml_Vec3:          ml_vec_3_mag_sq               \
)(v)
#define ml_vec_mag(v) _Generic((v),             \
    ml_Vec2:          ml_vec_2_mag,             \
    ml_Vec3:          ml_vec_3_mag              \
)(v)
#define ml_vec_scalar_add(v, s) _Generic((v),             \
    ml_Vec2:          ml_vec_2_scalar_add,             \
    ml_Vec3:          ml_vec_3_scalar_add              \
)(v, s)
#define ml_vec_scalar_sub(v, s) _Generic((v),             \
    ml_Vec2:          ml_vec_2_scalar_sub,             \
    ml_Vec3:          ml_vec_3_scalar_sub              \
)(v, s)
#define ml_vec_negate(v) _Generic((v),             \
...
MLLIB void ml_veclist3_push(ml_Vec3List *l, ml_Vec3 point){
    ...
    if(l->size == l->capacity){
        l->capacity *=2;
        void *temp = realloc(l->data, sizeof(ml_Vec3) * l->capacity);
        if(!temp){
            fprintf(stderr, "Ran out of memory!");
            exit(1);
        }
